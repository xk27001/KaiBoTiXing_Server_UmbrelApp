...
```text
ghcr.io/<github-user>/kai-bo-ti-xing:1.0.1-umbrel
```
...
```yaml
image: ghcr.io/xk27001/kai-bo-ti-xing:1.0.1-umbrel
```
...
docker build -f kaibotixing-server/Dockerfile -t ghcr.io/<github-user>/kai-bo-ti-xing:1.0.1-umbrel .
docker push ghcr.io/<github-user>/kai-bo-ti-xing:1.0.1-umbrel
```
...
```bash
docker manifest inspect ghcr.io/<github-user>/kai-bo-ti-xing:1.0.1-umbrel
```
...
```bash
docker login
docker build -f kaibotixing-server/Dockerfile -t <dockerhub-user>/kai-bo-ti-xing:1.0.1-umbrel .
docker push <dockerhub-user>/kai-bo-ti-xing:1.0.1-umbrel
```
...
```bash
cat > .env <<'EOF'
KBTX_IMAGE=<dockerhub-user>/kai-bo-ti-xing:1.0.1-umbrel
