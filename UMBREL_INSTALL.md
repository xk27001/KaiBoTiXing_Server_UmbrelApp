...
```text
ghcr.io/<github-user>/kai-bo-ti-xing:1.0.6-umbrel
```
...
```yaml
image: ghcr.io/xk27001/kai-bo-ti-xing:1.0.6-umbrel
```

改成：

```yaml
image: ghcr.io/<github-user>/kai-bo-ti-xing:1.0.6-umbrel
```
...
docker build -f kaibotixing-server/Dockerfile -t ghcr.io/<github-user>/kai-bo-ti-xing:1.0.6-umbrel .
docker push ghcr.io/<github-user>/kai-bo-ti-xing:1.0.6-umbrel
```
...
```bash
docker manifest inspect ghcr.io/<github-user>/kai-bo-ti-xing:1.0.6-umbrel
```
...
```yaml
tags: |
  ${{ steps.image.outputs.name }}:1.0.6-umbrel
...
```bash
docker login
docker build -f kaibotixing-server/Dockerfile -t <dockerhub-user>/kai-bo-ti-xing:1.0.6-umbrel .
docker push <dockerhub-user>/kai-bo-ti-xing:1.0.6-umbrel
```
...
```bash
cat > .env <<'EOF'
KBTX_IMAGE=<dockerhub-user>/kai-bo-ti-xing:1.0.6-umbrel
